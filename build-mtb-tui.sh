...
<rendertheme xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" map-background="#dcd5d0"
    ...
    <!-- LINE CASINGS -->
    <style-line id="unpaved_casing" stroke="#ada02a" cap="round"/>
    <style-line id="paved_cycleway_casing" stroke="#ffffff" cap="round"/>
    <style-line id="track_chasing" stroke="#ada02a" cap="round" width="1.1"/>
    <style-line id="MTB_0_chasing" stroke="#22AA22" cap="round" width="1.0"/>
    <style-line id="MTB_1_chasing" stroke="#88CC00" cap="round" width="1.0"/>
    <style-line id="MTB_2_chasing" stroke="#FFCC00" cap="round" width="1.0"/>
    <style-line id="MTB_3_chasing" stroke="#FF8800" cap="round" width="1.0"/>
    <style-line id="MTB_4_chasing" stroke="#DD0000" cap="round" width="1.0"/>
    <style-line id="MTB_5_chasing" stroke="#E40ADD" cap="round" width="1.0"/>
    ...
    <m e="way" k="waterway">
        <m v="ditch|drain" zoom-min="14">
            <line stroke="#FF0000" cap="round" width="3.0"/>
        </m>
...
build_map() {
    ...
    if [[ ! -f "${filtered_pbf}" ]] || [[ "${input_pbf}" -nt "${filtered_pbf}" ]]; then
        log_step "Filtering to mtb:scale ways, bare_rock areas, and ditches"
...
do_push_to_karoo() {
    ...
    local icons_dir="${DATA_DIR}/icons"
    if [[ -d "$icons_dir" ]]; then
        log_step "Pushing icon assets to Karoo"
        "$adb_cmd" shell "mkdir -p '${theme_path_base}/icons'" 2>/dev/null
        for icon_file in "$icons_dir"/*; do
            if [[ -f "$icon_file" ]]; then
                "$adb_cmd" push "$icon_file" "${theme_path_base}/icons/" 2>/dev/null
            fi
        done
        log_ok "Icon assets pushed successfully"
    fi
